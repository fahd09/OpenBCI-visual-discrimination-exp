...
parser.add_option("-d", "--duration",
                  dest="duration", type='int', default=5000,
...
while (time() - t_init) < options.duration:
    try:
        ...
        print(' EEG timestamps: ', timestamp[0])
        if timestamp:
            res.append(data)
        ...
        if inlet_marker:
            ...
            if timestamp:
                print(' MLR timestamps: ', timestamp*10)
                markers.append([marker, timestamp*10])
...
for marker in markers:
    ...
    abs_diffs = np.abs(marker[1] - timestamps)
    ix = np.argmin(abs_diffs)
    val = timestamps[ix]
